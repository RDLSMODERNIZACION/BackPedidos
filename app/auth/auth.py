        ...
        try:
            ...
            if body.items:
                cur.executemany("""
                    ...
                    values (%s, %s, %s, %s, %s);
                """, [
                    (ped["id"], it.descripcion, it.cantidad, it.unidad, it.precio_unitario)
...
@router.get("", summary="Lista de pedidos (vista)", tags=["pedidos"])
...
@router.get("/{pedido_id}", summary="Detalle (vista)", tags=["pedidos"])
