...
CUIT_RE = re.compile(r"\d{8,11}")  # tolerante a 8..11 dígitos (municipios a veces no usan guiones)
...
def _to_e164(phone: str) -> str:
    ...
    if len(digits) < 8 or len(digits) > 15:
        raise ValueError("telefono inválido (debe tener 8-15 dígitos en formato internacional)")
    return f"+{digits}"
...
def agregar_a_pedido(body: AddProveedorToPedidoIn):
    ...
    - Si viene 'telefono', lo setea/actualiza (conflicto -> 409 salvo que transfer_if_in_use=? no permitido aquí).
    ...
    with POOL.connection() as con, con.cursor() as cur:
        ...
        if body.set_adjudicado and body.rol == "adjudicatario":
            cur.execute("""
              UPDATE public.pedido SET adjudicado_a = %s WHERE id = %s
